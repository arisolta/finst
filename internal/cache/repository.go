...
func (r *Repository) GetFinancialStatements(ctx context.Context, ticker string) ([]model.FinancialStatement, bool, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT ticker, period_type, fiscal_year, fiscal_period, period_end_date,
		       ...
		       cash_and_equiv, total_debt, preferred_stock, total_equity, tax_expense, pretax_income, historical_price, cash_dividends_paid, stock_based_compensation, updated_at
	...
	for rows.Next() {
		...
		var rev, cor, gp, oi, da, ni, ds, eps, cfo, capex, cash, debt, pref, eq, tax, pretax, hp, divs, sbc sql.NullFloat64
		var endDate string

		if err := rows.Scan(
			&st.Ticker, &st.PeriodType, &st.FiscalYear, &st.FiscalPeriod, &endDate,
			&rev, &cor, &gp, &oi, &da, &ni, &ds, &eps, &cfo, &capex,
			&cash, &debt, &pref, &eq, &tax, &pretax, &hp, &divs, &sbc, &st.UpdatedAt,
		); err != nil {
			return nil, false, err
		}
		...
		st.StockBasedCompensation = sbc.Float64
...
func (r *Repository) SaveFinancialStatements(ctx context.Context, statements []model.FinancialStatement) error {
	...
	for _, s := range statements {
		...
		if _, err := stmt.ExecContext(ctx,
			...
			s.CashAndEquiv, s.TotalDebt, s.PreferredStock, s.TotalEquity, s.TaxExpense, s.PretaxIncome, s.HistoricalPrice, s.CashDividendsPaid, s.StockBasedCompensation, now,
		); err != nil {
			return err
		}
	}
